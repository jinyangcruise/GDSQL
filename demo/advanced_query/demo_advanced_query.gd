...
func _run_all() -> void:
    ...
    _demo_union_all()
    _demo_pagination()
    print("\nAll advanced query demos completed.\n")
...
func _setup_data() -> void:
    ...
    for g in [{"id":1,"name":"Crimson Blades","faction":"Alliance"},{"id":2,"name":"Shadow Walkers","faction":"Horde"},{"id":3,"name":"Arcane Circle","faction":"Alliance"}]:
        GDSQL.BaseDao.new().use_db(_db_path).insert_into("guild.gsql").primary_key("id",true).values(g).query()
    for h in [{"id":1,"name":"Arthur","hp":320,"guild_id":1},{"id":2,"name":"Merlin","hp":180,"guild_id":3},{"id":3,"name":"Robin","hp":220,"guild_id":1},{"id":4,"name":"Brunhild","hp":280,"guild_id":2},{"id":5,"name":"Luna","hp":200,"guild_id":3},{"id":6,"name":"Kael","hp":350,"guild_id":2}]:
        GDSQL.BaseDao.new().use_db(_db_path).insert_into("hero.gsql").primary_key("id",true).values(h).query()
    print("\n--- Data: 3 guilds, 6 heroes ---")

func _demo_left_join() -> void:
    print("\n--- LEFT JOIN ---")
    ...
    if ret and ret.ok():
        for r in ret.get_data(): print("  %-10s HP=%-4d Guild: %s (%s)" % [r.get("name",""), r.get("hp",0), r.get("guild",""), r.get("faction","")])

func _demo_aggregate_group_by() -> void:
    print("\n--- GROUP BY + COUNT/AVG ---")
    var dao := GDSQL.BaseDao.new()
    ...
    if ret and ret.ok():
        for r in ret.get_data(): print("  %-16s Heroes: %d  Avg HP: %.1f" % [r.get("name",""), r.get("cnt",0), r.get("avg_hp",0.0)])

func _demo_subquery() -> void:
    print("\n--- Subquery: Above average HP ---")
    var dao := GDSQL.BaseDao.new()
    ...
    if ret and ret.ok():
        for r in ret.get_data(): print("  %-10s HP=%d" % [r.get("name",""), r.get("hp",0)])

func _demo_union_all() -> void:
    print("\n--- UNION ALL ---")
    var dao := GDSQL.BaseDao.new()
    ...
    if ret and ret.ok():
        for r in ret.get_data(): print("  %-10s HP=%-4d (%s)" % [r.get("name",""), r.get("hp",0), r.get("tier","")])

func _demo_pagination() -> void:
    print("\n--- LIMIT + OFFSET (page 2) ---")
    var dao := GDSQL.BaseDao.new()
