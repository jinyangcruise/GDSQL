...
func _demo_subquery() -> void:
	...
	var dao = GDSQL.BaseDao.new()
	var ret = dao.use_db(_db_path).select("name, hp", false).from("hero.gsql").where("hp > (select avg(hp) from hero.gsql)").order_by("hp", GDSQL.ORDER_BY.DESC).query()
	if ret and ret.ok():
		for r in ret.get_data():
...
func _demo_pagination() -> void:
	...
	var dao = GDSQL.BaseDao.new()
	var ret = dao.use_db(_db_path).select("name, hp", false).from("hero.gsql").order_by("hp", GDSQL.ORDER_BY.DESC).limit(2, 1).query()
	if ret and ret.ok():
		for r in ret.get_data():
			print("  %-10s HP=%d" % [r.get("name", ""), r.get("hp", 0)])
