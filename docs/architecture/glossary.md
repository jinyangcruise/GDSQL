...
| `ModelAssistantDocument` | Editor model generation | Scene-backed table action that previews model sources, confirms generated-base replacement, preserves existing user scripts, stores the shared project model root plus an explicit class name per registration/database/table binding, and presents static compatibility plus catalog-inferred relationships. | `configure()` and generation signals | 🛠️ |
...
| `RelationshipDefinition` | Model API | Typed declared or catalog-inferred belongs-to, has-one, or has-many navigation captured by model registration for eager loading and editor inspection. | `belongs_to()`, `has_one()`, `has_many()`, typed keys | 🧪 |
