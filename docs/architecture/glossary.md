...
| `ModelAssistantDocument` | Editor model generation | Scene-backed table action that previews model sources, confirms generated-base replacement, preserves existing user scripts, stores the project model root, and presents static compatibility plus catalog-inferred relationships. | `configure()` and generation signals | 🛠️ |
