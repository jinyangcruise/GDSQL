...
| `QueryGraphEditor` | Editor query graph | Presents SELECT, INSERT, UPDATE, and DELETE nodes with in-node execution and titlebar removal; compiles the selected operation, requests execution through the workspace boundary, and presents table rows or mutation statistics without accessing storage. | `configure()`, `configure_actions()`, `request_query()`, `present_query_result()`, selected source accessors | 🛠️ |
| `QueryGraphNode` | Editor query graph presentation | Shared `GraphNode` base that extends Godot's native titlebar with an X button and a host for operation-specific controls, while emitting removal intent for the graph editor to coordinate. | `remove_requested`, `get_header_actions_host()`, `add_header_action()`, `set_close_enabled()` | 🛠️ |
