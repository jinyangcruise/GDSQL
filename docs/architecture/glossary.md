...
| `CrossRoleRelationshipHelper` | Editor model generation | Scene-backed Model Assistant component that matches supported save identifier columns to compatible content-model primary keys and copies one explicit `belongs_to()` entry. | `configure()` | 🛠️ |
...
| `RelationshipDefinition` | Model API | Typed declared or catalog-inferred belongs-to, has-one, has-many, or explicit many-to-many navigation captured by model registration for eager loading and editor inspection. Many-to-many definitions name a junction model and its source/target keys. | `belongs_to()`, `has_one()`, `has_many()`, `many_to_many()`, typed direct and junction keys | 🧪 |
