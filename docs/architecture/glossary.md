...
| `EditorResultGrid` | Editor query result | Graph-independent scene-backed Tree that renders typed rows, validates pending cell edits, tracks batched changes, presents Resource and expanded String editors, and requests bounded referenced-row options for unambiguous foreign-key cells without owning query execution, pagination, or mutation actions. | `configure()`, `configure_insert_draft()`, `render_page()`, `present_foreign_key_options()`, pending-change API | 🛠️ |
