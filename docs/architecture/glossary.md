...
| `McpInspectionService` | Editor agent integration | Projects supplied setup, registration, catalog, and schema state into deterministic bounded JSON-compatible results without reading rows, Controls, arbitrary files, or ConfigFile sections. | `get_capabilities()`, `inspect_setup()`, `inspect_schema()` | 🧪 |
