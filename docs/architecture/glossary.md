...
| `ModelCompatibilityReport` | Editor model inspection | Holds the registered model definition, structured table-compatibility diagnostics, and readable relationship role/table summaries. | `is_compatible()`, report fields | 🧪 |
| `ModelCompatibilityInspector` | Editor model inspection | Compares one user-owned model with an authoritative table and expected role without changing scripts or catalog state. | `inspect()` | 🧪 |
| `ModelAssistantDocument` | Editor model generation | Scene-backed table action that previews model sources, confirms generated-base replacement, preserves existing user scripts, stores the project model root, and presents read-only compatibility and relationship-role inspection. | `configure()` and generation signals | 🛠️ |
