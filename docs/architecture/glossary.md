...
| `QueryGraphSelectNode` | Graph frontend | Describes a SELECT source plus future named projections and canonical predicate without loading rows or accessing storage. | Database/table identity, `projections`, `predicate` | 🛠️ |
...
| `QueryGraphEditor` | Editor query graph | Presents canonical query composition through graph controls, owns document-scoped toolbar actions, compiles the selected SELECT operation, requests execution through the workspace boundary, and presents its returned table node without accessing storage. | `configure()`, `configure_actions()`, `request_query()`, `present_query_result()`, selected source accessors | 🛠️ |
