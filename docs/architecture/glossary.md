...
| `EditorResultGrid` | Editor query result | Graph-independent scene-backed Tree that renders typed rows with stable equal-width data columns, a fixed row-number gutter, horizontal overflow below their minimum width, and spreadsheet-style Tab/Enter cell movement; validates pending cell edits, tracks batched changes, presents Resource and expanded String editors, and requests bounded referenced-row options for same-database foreign keys or registered cross-role content identifiers without owning query execution, pagination, or mutation actions. | `configure()`, `configure_insert_draft()`, `render_page()`, reference-option presentation, pending-change API | 🛠️ |
