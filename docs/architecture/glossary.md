...
| `EditorResultGrid` | Editor query result | Graph-independent scene-backed Tree that renders typed rows with stable equal-width data columns, a fixed row-number gutter, horizontal overflow below their minimum width, and spreadsheet-style Tab/Enter cell movement; validates pending cell edits, tracks batched changes, presents Resource and expanded String editors, requests thumbnails only for supported visual Resources, applies asynchronous Resource presentation updates on the editor thread, and requests bounded referenced-row options for same-database foreign keys or registered cross-role content identifiers without owning query execution, pagination, or mutation actions. | `configure()`, `configure_insert_draft()`, `render_page()`, reference-option presentation, pending-change API | 🛠️ |
...
| `TableDataDocument` | Editor table data | Presents typed row values and emits canonical insert, duplicate, update, delete, refresh, same-database foreign-key lookup, and cross-role content-reference lookup intents without owning schema editing or query execution; only generated-identity rows with non-unique scalar values use atomic duplicate batches, while manual identities, copied unique constraints, or Resource values populate one editable insert draft from the first selected row. Draft Resources are deep-cloned. Constrained Resource columns expose validated scalar-property leaves to WHERE authoring. | `configure()`, `present_rows()`, reference presentation and row-intent signals | 🛠️ |
