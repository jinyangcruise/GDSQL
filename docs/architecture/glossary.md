...
| `EditorResultGrid` | Editor query result | Graph-independent scene-backed Tree that renders typed rows with stable equal-width data columns, a fixed row-number gutter, horizontal overflow below their minimum width, and spreadsheet-style Tab/Enter cell movement; validates pending cell edits, tracks batched changes, presents Resource and expanded String editors, and requests bounded referenced-row options for unambiguous foreign-key cells without owning query execution, pagination, or mutation actions. | `configure()`, `configure_insert_draft()`, `render_page()`, `present_foreign_key_options()`, pending-change API | 🛠️ |
...
| `ModelAssistantDocument` | Editor model generation | Scene-backed table action that previews model sources, confirms generated-base replacement, preserves existing user scripts, stores the shared project model root plus explicit class/role metadata per registration/database/table binding, presents static compatibility and catalog-inferred relationships, and produces copyable typed `save` → `content` relationship entries without creating cross-database constraints. | `configure()` and generation signals | 🛠️ |
| `CrossRoleRelationshipHelper` | Editor model generation | Scene-backed Model Assistant component that matches supported save identifier columns to compatible content-model primary keys and copies one explicit `references_one()` entry. | `configure()` | 🛠️ |
