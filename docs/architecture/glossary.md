...
| `EditorResultGrid` | Editor query result | Graph-independent scene-backed Tree that renders typed rows, validates pending cell edits, tracks batched changes, and presents Resource and expanded String editors without owning query execution, pagination, or mutation actions. | `configure()`, `configure_insert_draft()`, `render_page()`, pending-change API | 🛠️ |
