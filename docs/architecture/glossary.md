...
| `QueryGraph` | Graph frontend | Frontend-owned representation of query nodes and their connections; the initial executable structure validates one selected SELECT root. | `add_node()`, `get_nodes()`, `get_connections()`, `validate_structure()` | 🛠️ |
| `GraphQueryCompiler` | Graph frontend | Converts the currently supported SELECT-root `QueryGraph` into a canonical `SelectQuerySpec`, including named projections and a canonical predicate when present. | `compile(graph)` | 🛠️ |
...
| `ConfigFileDatabaseExplorer` | Database discovery backend | Reads ConfigFile database catalogs, typed column schema summaries, and reserved table headers without materializing rows. | `inspect_root()` | 🧪 |
...
| `TableInspection` | Database discovery metadata | Reports table existence, row-count header, typed inspected columns, and index count without containing row values. | Typed inspection fields, `get_column()` | 🧪 |
...
| `QueryGraphEditor` | Editor query graph | Presents SELECT nodes with in-node execution, multi-column projection, composed typed `GDSQLExpr` WHERE predicates, and right-click removal; compiles the selected operation, requests execution through the workspace boundary, and presents its returned table node without accessing storage. | `configure()`, `configure_actions()`, `request_query()`, `present_query_result()`, selected source accessors | 🛠️ |
...
| `QueryTableResultNode` | Editor query result | Presents `QueryResult` rows using the returned schema, marks changed rows dirty, permits primary-key-safe edits, permits inserts only for full-column results, and emits row mutation intents. | `present()`, `can_add_rows()`, `has_dirty_rows()`, `add_empty_row()` | 🛠️ |
