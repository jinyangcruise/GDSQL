...
| `WhereExpressionEditor` | Editor query expression | Reusable predicate editor for SELECT, UPDATE, and DELETE graph operations; presents ordinary columns and validated Resource scalar leaves, composes ordered typed conditions with `AND`, `OR`, and per-condition `NOT`, returns canonical expressions and structured diagnostics, and never evaluates predicates or accesses storage. | `configure()`, `build_expression()`, `get_summary()` | 🛠️ |
