...
| `Workspace` | Editor workspace | Hosts scene-backed documents behind editor-style menus and tabs, activates document action contexts, confirms discard before closing documents with unsaved changes, keeps registration identity separate from logical titles, and routes database, save-slot, schema, typed row, and model-assistant intents. | `show_welcome()`, `show_save_slots()`, `show_database()`, `show_table()`, `show_model_assistant()`, tab lifecycle | 🛠️ |
