...
| `EditorRowBatch` | Editor row mutation | Validates one table-scoped update or delete batch, converts it into canonical queries, and executes the queries through one public database transaction. | `build_updates()`, `build_deletes()`, `execute()` | 🧪 |
...
| `EditorMutationHistory` | Editor row history | Maintains bounded session-only Undo/Redo stacks and moves entries only after the controller confirms successful inverse execution. | `record()`, `get_undo_entry()`, `mark_undone()`, `mark_redone()` | 🧪 |
