...
| `RuntimeFactory` | Runtime composition | Assembles storage-specific runtime graphs, opens durable registrations, and bootstraps the supported database/model/persistence session; in-memory registrations hydrate ConfigFile rows as a clean working set. | `create_default()`, `create_in_memory()`, `open_registration()`, `bootstrap()` | 🧪 |
