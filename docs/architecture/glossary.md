...
| `QueryGraphEditor` | Editor query graph | Presents SELECT nodes with in-node execution, multi-column projection, one typed `GDSQLExpr` WHERE predicate, and right-click removal; compiles the selected operation, requests execution through the workspace boundary, and presents its returned table node without accessing storage. | `configure()`, `configure_actions()`, `request_query()`, `present_query_result()`, selected source accessors | 🛠️ |
