...
| `RuntimeNode` | Godot runtime adapter | Optional scene or autoload that bootstraps the selected direct or managed profile, exposes managed activation results plus role/model/save-slot delegates, schedules periodic dirty checkpoints, and checkpoints synchronously on pause or exit without owning database logic. | `start()`, `get_content_activation_result()`, `database()`, `register_model()`, `select_save_slot()`, `checkpoint_now()`, `stop()`, runtime signals | 🧪 |
