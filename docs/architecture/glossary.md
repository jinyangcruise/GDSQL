...
| `ModelSourceGenerator` | Editor model generation | Converts a catalog table and logical role into separate generated and user-owned `@tool` GDScript sources so explicitly selected content models can support editor preview, without accessing Controls, files, or catalog mutation services. | `build()` | 🧪 |
...
| `EditorContentPreviewService` | Editor content preview | Registers one explicitly selected content model in an injected model context, converts a textual stable identifier to its declared primary-key type, and returns the read-only materialized row with structured diagnostics. | `load()` | 🛠️ |
| `ContentPreviewBinding` | Editor content preview | Maps one content-model property to one property on a node inside a transient preview scene. | `model_property`, `target_node`, `target_property` | 🛠️ |
| `ContentPreviewAdapter` | Editor content preview | Opt-in tool node that uses the configured Direct or Managed content role for a manual row lookup, applies bindings only to an internal ownerless `PackedScene` instance, reports structured status, and immediately releases its temporary runtime session. | `refresh_preview()`, `clear_preview()`, `get_last_result()` | 🛠️ |
