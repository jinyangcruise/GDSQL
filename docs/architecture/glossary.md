...
|---|---|---|---|---|
| `Database` | Public API | Main user-facing entry point for creating, opening, renaming, unregistering, and destructively dropping a database; managing and transactionally truncating its tables; executing canonical query specs; and running callback-scoped transactions. | `create()`, `open()`, `rename()`, `unregister()`, `drop()`, `truncate_table()`, table administration, `query()`, `execute()`, `transaction()` | 🧪 |
| `DatabaseContext` | Runtime facade | Coordinates catalog administration, transactional table truncation, validation, binding, planning, execution, shared-session transactions, and result materialization. | Database and table administration methods, `truncate_table()`, `execute(query)`, `transaction(callback)`, `prepare(query)` | 🧪 |
...
| `FunctionExpression` | Expression model | Describes a validated scalar or aggregate function invocation and retains a bound return type when it depends on validated arguments. | Access to name, arguments, resolved return type, and `accept(visitor)` | 🧪 |
...
| `BoundColumnExpression` | Binding | Column expression resolved to stable table and column identifiers, source occurrence qualifier, data type, nullability, and any Resource type constraint required by semantic validation. | Access to table ID, column ID, source qualifier, data type, nullability, and Resource type | 🧪 |
...
| `TransactionManager` | Execution | Coordinates storage sessions, foreign-key validation of the final transactional state, commits, and rollbacks. | `begin()`, `commit()`, `rollback()` | 🧪 |
| `ForeignKeyConstraintValidator` | Execution integrity | Validates every same-database foreign key affected by a storage session against final effective rows before commit, including references from otherwise untouched tables. | `validate(session)` | 🧪 |
...
| `ConfigFileCatalogAdministrationService` | Catalog backend | Persists database registrations, preserves physical files during unregistration, synchronizes ConfigFile-backed schemas and row storage, and rejects table, column, index, or uniqueness changes that would invalidate incoming foreign keys. | CatalogAdministrationService implementation | 🧪 |
...
| `ForeignKeyDefinition` | Catalog | Describes one named, single-column, same-database foreign key and its update/delete policy independently from model navigation or runtime roles. ConfigFile schemas and effective-content copies preserve the metadata; catalog alterations resolve supported, matching, unique targets and reject orphaned existing values; transaction commit enforces the final row state. | Local column, referenced table/column, `supports_column_type()`, `references_local_column()`, `is_equivalent_to()` | 🧪 |
| `TableDefinition` | Catalog | Typed definition of a table, its columns, primary key, indexes, foreign keys, and common timestamp helpers. | `add_column()`, `add_index()`, `add_foreign_key()`, `add_timestamps()`, `get_column()`, `get_primary_key()`, `get_index()`, `has_unique_key()`, `get_foreign_key()` | 🧪 |
| `ColumnDefinition` | Catalog | Typed definition of one table column, including an optional static default, generated-value policy, integer primary-key auto-increment, and mandatory concrete type plus ownership semantics for `TYPE_OBJECT`. | `set_default()`, `clear_default()`, `has_default()`, `get_default_value()`, `accepts_value()`, `has_valid_type_constraint()`, `display_type_name()`, `created_at()`, `updated_at()` | 🧪 |
...
| `ResourceOwnership` | Catalog | Defines whether a Resource column owns an independent value or references an external asset, without prescribing backend layout. | `OWNED`, `REFERENCED`, stable ID conversion | 🧪 |
| `ResourcePropertyDefinition` | Catalog | Identifies one Inspector-visible scalar leaf path and its Variant type for a constrained Resource family. | `serialized_path()`, `display_path()` | 🧪 |
| `ResourcePropertyCatalog` | Catalog | Discovers and resolves bounded filterable Resource leaves from static native or script property metadata; supported compound Variants expose scalar components but not their container value. | `list_filterable_leaves()`, `resolve_filterable_leaf()` | 🧪 |
...
| `TableAlteration` | Catalog | Typed intent for safe column metadata, display order, column lifecycle, index changes, and foreign-key changes. Direct data-type replacement is expressed as add, migrate, and drop operations. | Column add/rename/drop/reorder, default, nullability, uniqueness, generation, auto-increment, index, and foreign-key factories | 🧪 |
...
| `InMemoryTableStorage` | Storage backend | Provides authoritative temporary table storage with transaction-local visibility, exact generated-key hydration, transactional truncation, committed dirty-version tracking, constraints, and scan-backed index capabilities. | TableStorage implementation, `load_table()`, `is_dirty()`, `get_dirty_tables()`, `mark_checkpointed()` | 🧪 |
| `DatabaseRegistry` | Database lifecycle | Owns loaded durable registration metadata, registers selected open database handles, resolves replaceable logical roles, atomically installs runtime-local role databases, and delegates snapshot persistence. | Registration listing and lookup, `register()`, `resolve()`, `replace_role_database()`, role binding, `load_snapshot()`, `save_snapshot()` | 🧪 |
...
| `RuntimeFactory` | Runtime composition | Assembles storage-specific runtime graphs, opens runtime registrations, exposes a durable editor-authoring composition without changing stored backend policy, bootstraps with profile-appropriate diagnostics, and coordinates configured managed-content activation; in-memory runtimes hydrate ConfigFile rows as a clean working set. | `create_default()`, `create_in_memory()`, `open_registration()`, `open_authoring_registration()`, `bootstrap()`, `activate_managed_content()` | 🧪 |
| `RuntimeSession` | Runtime facade | Exposes role-based database resolution, the default model context, checkpoint-safe save-slot selection, and explicit checkpoint operations over one bootstrapped registry. | Service getters, `database()`, `register_model()`, `get_active_save_slot()`, `select_save_slot()`, `select_role()`, `checkpoint_role()`, `checkpoint_save()`, `shutdown()` | 🧪 |
...
| `InMemoryCheckpointTarget` | Runtime persistence | Adapts authoritative in-memory storage to `CheckpointTarget` and synchronizes each dirty table, including its exact next generated key, into an injected durable `TableStorage` backend. | `is_dirty()`, `checkpoint()` | 🧪 |
...
| `ContentOverlayLoader` | Runtime content loading | Validates and deterministically combines immutable base content with enabled package layers into one effective-content snapshot. | `build_effective_database()` | 🧪 |
| `ContentDatabaseSnapshot` | Runtime content result | Owns copied effective database definitions and row snapshots before cache materialization or role replacement. | `definition`, `get_table_definition()`, `get_table()` | 🧪 |
| `ContentOverlayResult` | Runtime content result | Carries the effective snapshot, applied row provenance, typed override conflicts, and structured diagnostics. | `database`, `provenance`, `conflicts`, `get_row_history()`, `get_effective_origin()` | 🧪 |
| `ContentRowProvenance` | Runtime content diagnostics | Identifies the package ID, version, and operation responsible for one step in a stable row identity's history. | Typed source and operation fields, `is_removal()` | 🧪 |
| `ContentRowConflict` | Runtime content diagnostics | Associates an existing row origin with the later package origin that deterministically replaced it. | `previous`, `replacement`, table and row identity | 🧪 |
| `ContentPackageLayer` | Runtime content loading | Holds one decoded package's selected database schemas and typed ordered row operations. | `table_definitions`, `operations`, `add_upsert()`, `add_removal()` | 🧪 |
| `ContentRowOperation` | Runtime content loading | Represents one stable-identity upsert or explicit removal without embedding magic fields in row data. | `UPSERT`, `REMOVE`, `identity`, `row` | 🧪 |
| `ContentPackageLayerReader` | Runtime content loading | Abstract boundary for decoding one package's selected logical database into a typed layer. | `read_layer()` | 🧪 |
| `ConfigFileContentPackageLayerReader` | Runtime content backend | Reads package catalog schemas and rows and decodes explicit removals from `overlays.cfg`. | `read_layer()` | 🧪 |
| `ContentPackageKind` | Runtime content metadata | Distinguishes base-game, DLC, and mod packages using stable manifest identifiers. | `BASE_GAME`, `DLC`, `MOD`, identifier conversion | 🧪 |
| `ContentPackageDependency` | Runtime content metadata | Associates one required package identifier with its declared version constraint. | Typed package ID and version constraint | 🧪 |
| `ContentPackageManifest` | Runtime content metadata | Describes one immutable content package through identity, semantic version, kind, priority, package-relative roots, dependencies, and explicit ordering requirements. | Typed manifest fields | 🧪 |
| `ContentPackageManifestValidator` | Runtime content validation | Validates one manifest's local identity, semantic version, safe relative paths, dependencies, and non-conflicting order declarations without discovering other packages. | `validate()` with structured diagnostics | 🧪 |
| `ContentPackageManifestStore` | Runtime content metadata persistence | Abstract boundary for loading a typed package manifest from an explicitly supplied package source root. | `load_manifest()` | 🧪 |
| `ConfigFileContentPackageManifestStore` | Runtime content metadata backend | Decodes `manifest.cfg` sections into typed package metadata and delegates local invariant checks to the manifest validator. | `load_manifest()` | 🧪 |
| `ConfigFileContentPackageScaffolder` | Editor setup backend | Creates a package's data/assets envelope and fills only absent ConfigFile manifest fields before validating the result. | `scaffold()` | 🧪 |
| `SemanticVersion` | Runtime content compatibility | Parses and compares package semantic versions, including prerelease precedence while ignoring build metadata for ordering. | `parse()`, `compare()` | 🧪 |
| `SemanticVersionConstraint` | Runtime content compatibility | Evaluates exact, comparator, caret, tilde, and conjunctive package-version constraints. | `matches()` | 🧪 |
| `ContentPackageSource` | Runtime content discovery metadata | Associates one validated package manifest with its immutable source root and resolves package-relative data and asset roots. | `manifest`, `package_root`, `get_data_root()`, `get_assets_root()` | 🧪 |
| `ContentPackageDiscovery` | Runtime content discovery | Abstract boundary for discovering a base package and optional packages from explicitly supplied source locations. | `discover()` | 🧪 |
| `ConfigFileContentPackageDiscovery` | Runtime content discovery backend | Deterministically enumerates direct directory packages and nested `content/` layouts, decoding each through the injected manifest store. | `discover()` | 🧪 |
| `ContentPackageResolver` | Runtime content ordering | Selects one base plus explicitly enabled packages, validates dependencies and semantic-version constraints, and produces a stable topological precedence order. | `resolve()` | 🧪 |
| `ContentPackageResolutionResult` | Runtime content ordering result | Carries the deterministic ordered package sources and structured dependency or cycle diagnostics. | `ordered_packages`, `get_package()`, operation result API | 🧪 |
| `ContentCacheManager` | Runtime content loading | Compares the expected package fingerprint manifest with the disposable cache and rebuilds through the overlay loader on a miss. | `ensure_cache()` | 🧪 |
| `ContentCacheManifest` | Runtime content loading | Fingerprints source/effective database identity, manifest format, package order, versions, and content hashes for a disposable effective-content cache. | Typed fields, `is_equivalent_to()` | 🧪 |
| `ContentPackageFingerprint` | Runtime content loading | Identifies one resolved package through stable ID, semantic version, and content hash. | Typed fields, `is_equivalent_to()` | 🧪 |
| `ContentPackageFingerprintProvider` | Runtime content loading | Abstract boundary for hashing an immutable package source without coupling cache orchestration to its representation. | `fingerprint()` | 🧪 |
| `ContentCacheStore` | Runtime content loading | Abstract persistence boundary for loading a cache manifest, checking the cached database, and replacing disposable cache contents. | `load_manifest()`, `has_database()`, `replace()` | 🧪 |
| `ConfigFileContentCacheStore` | Runtime content backend | Materializes the effective snapshot and manifest in a staging directory before replacing the bounded ConfigFile cache. | ContentCacheStore implementation | 🧪 |
| `ConfigFileContentPackageFingerprintProvider` | Runtime content backend | Computes SHA-256 over sorted package-relative paths and file bytes. | ContentPackageFingerprintProvider implementation | 🧪 |
| `ContentCacheResult` | Runtime content result | Reports cache hit or rebuild status, the compatible manifest, cache root, and structured diagnostics. | `status`, `manifest`, `cache_root`, `was_rebuilt()` | 🧪 |
| `ContentActivationResult` | Runtime content result | Reports cache preparation together with the effective database installed for the runtime `content` role. | Database result API, `cache_result`, `was_rebuilt()` | 🧪 |
| `SaveContentManifest` | Runtime save compatibility | Records the effective database and ordered package fingerprints a managed save expects. | `from_cache_manifest()`, typed manifest fields | 🧪 |
| `SaveContentManifestStore` | Runtime save compatibility | Abstract persistence boundary for one save's expected content package set. | `load_manifest()`, `save_manifest()` | 🧪 |
| `ConfigFileSaveContentManifestStore` | Runtime content backend | Stores validated save content expectations in `content_manifest.cfg` beside a save catalog. | SaveContentManifestStore implementation | 🧪 |
| `SaveContentCompatibilityInspector` | Runtime save compatibility | Compares save expectations with the active effective-content manifest without choosing a game policy. | `inspect()` | 🧪 |
| `SaveContentCompatibilityReport` | Runtime save compatibility result | Separates unavailable, untracked, exact, changed, and missing-package states with typed differences and diagnostics. | Status, package differences, `can_resolve_expected_packages()`, `requires_policy_decision()` | 🧪 |
| `SaveContentPackageChange` | Runtime save compatibility result | Associates one saved package fingerprint with the active fingerprint that replaced it. | `expected`, `active` | 🧪 |
...
| `RuntimeNode` | Godot runtime adapter | Optional scene or autoload that bootstraps the selected direct or managed profile, exposes managed activation and active-save compatibility results plus role/model/save-slot delegates, schedules periodic dirty checkpoints, and checkpoints synchronously on pause or exit without owning database or game-policy logic. | `start()`, `get_content_activation_result()`, `get_save_content_compatibility_report()`, `database()`, `register_model()`, `select_save_slot()`, `checkpoint_now()`, `stop()`, runtime signals | 🧪 |
...
| `ManagedContentConfiguration` | Runtime setup metadata | Carries the base package root, optional package-container roots, and enabled package IDs shared by editor setup and runtime activation. | Typed fields and `create_default()` | 🧪 |
| `ManagedContentConfigurationStore` | Runtime setup persistence | Storage-independent boundary for loading and saving managed-content package configuration. | `load_configuration()`, `save_configuration()` | 🧪 |
| `ConfigFileManagedContentConfigurationStore` | Runtime setup backend | Persists typed managed-content configuration in project settings while preserving the setup profile and unrelated sections. | ManagedContentConfigurationStore implementation | 🧪 |
...
| `WorkbenchSession` | Editor coordination | Holds one selected registration, opens its durable authoring source even when its runtime backend is in-memory, and owns the catalog snapshot, selected table, current page, and pending schema preview without depending on Controls. | `open_registration()`, `refresh_catalog()`, `select_table()`, `load_rows()`, preview and apply methods | 🧪 |
| `SaveSlotDeletionPlan` | Editor coordination | Validates and describes one exact standard save-slot database deletion without accessing storage or deleting files; rejects the shared save root, custom roots, and nested roots. | `build()` and immutable target fields | 🧪 |
| `EditorRowBatch` | Editor row mutation | Validates one table-scoped insert, update, or delete batch, converts it into canonical queries, executes the queries through one public database transaction, and creates an optional non-Resource update history snapshot from editor-supplied prior values. | `build_inserts()`, `build_updates()`, `build_deletes()`, `create_history_entry()`, `execute()` | 🧪 |
| `EditorMutationHistoryEntry` | Editor row history | Owns before/after row snapshots and target identity for one successfully committed editor mutation batch. | Target fields, operation, snapshots, and `get_summary()` | 🧪 |
| `EditorMutationHistory` | Editor row history | Maintains bounded in-memory Undo/Redo stacks for the current editor session only and moves entries only after the controller confirms successful inverse execution; it is never persisted across editor restarts. | `record()`, `get_undo_entry()`, `mark_undone()`, `mark_redone()` | 🧪 |
...
| `EditorCommandPalette` | Editor navigation | Projects lightweight registered database, table, and column inspection metadata into Godot's existing command palette and delegates selected entries through the editor action hub. | `refresh()`, `clear()` | 🛠️ |
...
| `EditorController` | Editor integration | Registers editor actions and coordinates automatic discovery, create-or-load database behavior, catalog mutations, bounded in-memory table Undo/Redo, editor filesystem refresh, and selection with the active editor surfaces, independently from Godot dock placement. | `load_workspace()`, `ensure_workspace_loaded()`, `shutdown()` | 🛠️ |
| `McpInspectionService` | Editor agent integration | Projects supplied setup, registration, catalog, schema, and model-binding state into deterministic bounded JSON-compatible results without reading rows, Controls, arbitrary files, or ConfigFile sections. | `get_capabilities()`, `inspect_setup()`, `inspect_schema()`, `inspect_models()` | 🧪 |
| `McpModelInspectionService` | Editor agent integration | Statically inspects persisted model bindings, generated-script compatibility, catalog-inferred relationships, and registered cross-role references without instantiating models or executing user relationship declarations. | `inspect()` | 🧪 |
| `GodotAiMcpAdapter` | Optional editor integration | Dynamically registers promoted read-only GDSQL tool specs with the optional Godot-AI bridge, survives either plugin load order, unregisters on teardown, validates transport arguments, and translates inspection results without becoming a runtime dependency. Tool visibility remains controlled by Godot-AI per project. | Construction and `shutdown()` | 🛠️ |
...
| `Workspace` | Editor workspace | Hosts scene-backed documents behind editor-style menus and tabs, activates document action contexts, confirms discard before closing documents with unsaved changes, keeps registration identity separate from logical titles, and routes database, managed-content, save-slot, schema, typed row, and model-assistant intents. | `show_welcome()`, `show_managed_content()`, `show_save_slots()`, `show_database()`, `show_table()`, `show_model_assistant()`, tab lifecycle | 🛠️ |
| `SaveSlotsDocument` | Editor runtime setup | Discovers standard save-slot roots, displays the durable active save binding, and delegates creation, selection, opening, unregister-and-keep-files, and confirmed validated deletion through shared editor actions. | `configure()`, `refresh_slots()` | 🛠️ |
| `ManagedContentDocument` | Editor runtime setup | Configures package source roots and enabled IDs, validates deterministic resolution, builds the disposable effective cache, reports active-save compatibility, and confirms package-expectation recording. | `configure()`, `refresh_status()` | 🛠️ |
...
| `EditorWhereField` | Editor query expression | Typed selectable WHERE target representing either a table column or one validated Resource scalar-leaf path. | `key()`, `display_name()`, `expression()` | 🛠️ |
| `EditorWhereFieldCatalog` | Editor query expression | Converts catalog columns into ordinary fields plus bounded Resource-property leaves without exposing compound Resource values as filter operands. | `build()` | 🛠️ |
| `WhereExpressionEditor` | Editor query expression | Reusable predicate editor for table filtering and SELECT, UPDATE, and DELETE graph operations; presents ordinary columns and validated Resource scalar leaves, delegates explicit precedence to recursive groups, returns canonical expressions and structured diagnostics, and never evaluates predicates or accesses storage. | `configure()`, `build_expression()`, `get_summary()` | 🛠️ |
| `WhereExpressionGroup` | Editor query expression | Scene-backed recursive WHERE group with ordered condition/group children, bounded nesting, `AND`/`OR` connectors, group-level `NOT`, reordering, and collapsible summaries. | `configure()`, `build_expression()`, `get_summary()`, `get_condition_count()` | 🛠️ |
| `EditorResultGrid` | Editor query result | Graph-independent scene-backed Tree that renders typed rows with stable equal-width data columns, a fixed row-number gutter, horizontal overflow below their minimum width, and spreadsheet-style Tab/Enter cell movement; validates pending cell edits, tracks owned Resource property edits and referenced Resource replacements, presents Resource and expanded String editors, requests thumbnails only for supported visual Resources, applies asynchronous Resource presentation updates on the editor thread, and requests bounded referenced-row options for same-database foreign keys or registered cross-role content identifiers without owning query execution, pagination, or mutation actions. | `configure()`, `configure_insert_draft()`, `render_page()`, reference-option presentation, pending-change API | 🛠️ |
...
| `EditorColumnEditor` | Editor schema presentation | Coordinates a fixed scene-authored header, reusable column rows with extensible context actions, reversible removal state, and typed local drafts; produces column definitions or alterations without accessing storage. | `configure_new_table()`, `configure_existing()`, `add_draft_column()`, context/removal methods, `build_definitions()`, `build_alterations()` | 🛠️ |
| `EditorColumnDraft` | Editor schema state | Holds one mutable local column edit independently from Controls and converts it into validation messages, a new `ColumnDefinition`, or alterations against its original definition; persisted Resource constraints retain type metadata without reconstructing artificial native prototypes. | `create_new()`, `from_definition()`, `get_validation_errors()`, `build_definition()`, `build_alterations()` | 🛠️ |
...
| `EditorForeignKeyDraftRow` | Editor schema presentation | Scene-backed same-database foreign-key authoring row that excludes the source table, narrows searchable local, target-table, and unique-column choices by supported exact key type, generates a deterministic input-derived constraint name, and builds typed catalog metadata without accessing storage. | `configure()`, `refresh_context()`, `build_definition()`, `get_validation_errors()` | 🛠️ |
| `TableDataDocument` | Editor table data | Presents typed row values and emits canonical insert, duplicate, update, delete, refresh, same-database foreign-key lookup, and cross-role content-reference lookup intents without owning schema editing or query execution; only generated-identity rows with non-unique scalar or referenced Resource values use atomic duplicate batches, while manual identities, copied unique constraints, or owned Resource values populate one editable insert draft from the first selected row. Draft owned Resources are deep-cloned; referenced Resources retain their asset identity. Constrained Resource columns expose validated scalar-property leaves to WHERE authoring. | `configure()`, `present_rows()`, reference presentation and row-intent signals | 🛠️ |
| `ModelSource` | Editor model generation | Typed preview output containing generated-base and user-model paths and source text. | Generation output fields | 🧪 |
| `ModelSourceGenerator` | Editor model generation | Converts a catalog table and logical role into separate generated and user-owned GDScript sources without accessing Controls, files, or catalog mutation services. | `build()` | 🧪 |
| `ModelCompatibilityReport` | Editor model inspection | Holds structured static compatibility diagnostics without retaining or executing a runtime model definition. | `is_compatible()`, diagnostics | 🧪 |
| `ModelCompatibilityInspector` | Editor model inspection | Compares one user-owned Script's generated base, role inheritance, and static properties with an authoritative table without instantiating it or changing scripts or catalog state. | `inspect()` | 🧪 |
| `ModelAssistantDocument` | Editor model generation | Scene-backed table action that previews model sources, confirms generated-base replacement, preserves existing user scripts, opens the user-owned model in Godot's Script editor, stores the shared project model root plus explicit class/role metadata per registration/database/table binding, presents static compatibility and catalog-inferred relationships, and registers copyable typed `save` → `content` relationship entries without creating cross-database constraints. | `configure()` and generation signals | 🛠️ |
| `CrossRoleRelationshipHelper` | Editor model generation | Scene-backed Model Assistant component that matches supported save identifier columns to compatible content-model primary keys, stores and lists editor picker metadata, copies explicit runtime `references_one()` entries, and removes mistaken picker bindings without editing user model code. | `configure()`, `reference_registered` | 🛠️ |
| `EditorContentReference` | Editor model generation | Typed editor-only mapping from one save-table identifier column to a content registration, table, key, and model binding. It enables authoring lookup without representing the edge as a catalog foreign key. | Source and target identities, relationship name | 🧪 |
| `EditorContentReferenceStore` | Editor model generation | Serializes and removes editor content-reference mappings at the project-tool settings boundary and returns typed mappings to table documents. | `save()`, `remove()`, `load_for_table()` | 🧪 |
| `EditorVariantValueField` | Editor value input | Edits one catalog-typed Godot Variant value, keeps nullable inputs interactive while preserving explicit null, enters text edit mode on focus, and delegates Resource selection and inspection to native editor controls while respecting owned-copy versus referenced-asset semantics. | `configure()`, `set_value_editable()`, `get_value_result()` | 🛠️ |
| `ActivityPanel` | Editor feedback | Presents operation outcomes and structured diagnostics through a bounded entry list with severity styling, automatic latest-entry scrolling, error focus, a dock severity indicator, Unix-millisecond selection IDs, and context actions. | `append_result()`, `append_message()`, `indicator_changed`, `get_entry()`, `select_entry()`, `delete_entry()`, `copy_entry_message()`, `clear()`, `log_limit` | 🛠️ |
...
| Name | Domain | Responsibility | Principal API | State |
|---|---|---|---|---|
| `Model` | Model API | Shared base for role-scoped metadata, materialized identity, source-database provenance, change tracking, context retention, loaded relationships, and persisted-row operations. It rejects stale instance operations after a role changes databases. | metadata, `relationships()`, `get_related()`, `is_persisted()`, `save()`, `refresh()`, `delete()` | 🧪 |
...
| `SaveModel` | Model API | Mutable model for durable, slot-scoped saved state, bound through the model registry to the active save database. It does not represent transient runtime state, and save-slot management remains in the database registry. | Query, refresh, save, and delete | ✅ |
...
| `ModelDefinition` | Model metadata | Captures a registered model script, logical role, table, primary key, access mode, declared relationships, and catalog-inferred relationships; declared names take precedence. | Typed definition fields, `get_relationship()`, `replace_inferred_relationships()` | 🧪 |
| `ModelRegistry` | Model API | Registers model classes, resolves their typed metadata and logical roles through `DatabaseRegistry`, and refreshes default same-database navigation as related models register. | `register()`, `resolve_model()`, `resolve_role()` | 🧪 |
