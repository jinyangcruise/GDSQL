...
| `EditorRowBatch` | Editor row mutation | Validates one table-scoped update or delete batch, converts it into canonical queries, executes the queries through one public database transaction, and creates an optional non-Resource update history snapshot from editor-supplied prior values. | `build_updates()`, `build_deletes()`, `create_history_entry()`, `execute()` | 🧪 |
...
| `EditorResultGrid` | Editor query result | Graph-independent scene-backed Tree that renders typed rows with stable equal-width data columns, a fixed row-number gutter, and horizontal overflow below their minimum width; validates pending cell edits, tracks batched changes, presents Resource and expanded String editors, and requests bounded referenced-row options for unambiguous foreign-key cells without owning query execution, pagination, or mutation actions. | `configure()`, `configure_insert_draft()`, `render_page()`, `present_foreign_key_options()`, pending-change API | 🛠️ |
...
| `TableDataDocument` | Editor table data | Presents typed row values and emits canonical insert, update, delete, refresh, and foreign-key reference lookup intents without owning schema editing or query execution. | `configure()`, `present_rows()`, `present_reference_rows()` and row-intent signals | 🛠️ |
