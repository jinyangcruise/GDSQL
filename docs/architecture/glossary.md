...
| `RuntimeFactory` | Runtime composition | Assembles storage-specific runtime graphs, opens durable registrations, bootstraps with profile-appropriate diagnostics, and coordinates configured managed-content activation; in-memory registrations hydrate ConfigFile rows as a clean working set. | `create_default()`, `create_in_memory()`, `open_registration()`, `bootstrap()`, `activate_managed_content()` | 🧪 |
