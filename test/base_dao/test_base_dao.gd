...
func t_select_order():
    var dao := GDSQL.BaseDao.new()
    var r = dao.use_db(_db).select("val").from(_t).order_by("val", GDSQL.ORDER_BY.DESC).query()
...
func t_update():
    ...
    var d2 := GDSQL.BaseDao.new()
    var r2 = d2.use_db(_db).select("val").from(_t).where("name == 's0'").query()
