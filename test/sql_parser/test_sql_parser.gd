...
func _ready():
    print("\n"+"="*60+"\n  Test: SQL Parser\n"+"="*60+"\n")
    t_parse_simple(); t_parse_where(); t_parse_order_limit(); t_parse_insert()
    t_parse_update(); t_parse_delete(); t_parse_left_join(); t_parse_aggregate()
    _summary()
func ok(d): _pass+=1; print("  [PASS] "+d)
func nt(d,s=""): _fail+=1; printerr("  [FAIL] "+d+" - "+s)
func _summary(): print("\n  %d passed, %d failed\n%s\n" % [_pass,_fail,"="*60])
func _parse(s): var r = GDSQL.SQLParser.parse_to_dao(s); return r != null
func t_parse_simple():
