	...
	if p_event is InputEventMouseButton:
		...
		# 如果按了CTRL，则自动跳到下一个/上一个出现的位置
			...
			if to_highlight_col > 0:
				var to_highlight_info = global_pos_to_line_col(
					to_highlight_col,
				...
				text_editor.select(
					to_highlight_info.start_line,
					to_highlight_info.start_col,
					to_highlight_info.end_line,
					to_highlight_info.end_col,
				)
