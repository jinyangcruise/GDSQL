	...
	if what == NOTIFICATION_APPLICATION_FOCUS_IN or what == NOTIFICATION_VISIBILITY_CHANGED:
		...
		for path: String in GDSQL.ConfManager._conf_modified_time:
			if not GDSQL.GDSQLUtils.file_exists(path):
				...
				ti.add_button(
					1,
					get_theme_icon("Clear", "EditorIcons"),
					0,
					false,
					tr("Clear cache"),
				)
...
func refresh() -> void:
	...
	for db_name in mgr.databases:
		...
		for t in data["tables"]:
			...
			popup_menu_create_table_like_tables.set_item_metadata(
				idx_1,
				{
					"db_name": db_name,
					"table_name": t,
			...
			popup_menu_create_table_like_table_item.set_item_metadata(
				idx_2,
				{
					"db_name": db_name,
					"table_name": t,
...
func add_database(db_name: String, data: Dictionary) -> TreeItem:
	...
	database_item.add_button(
		0,
		load("res://addons/gdsql/img/folder.svg"),
...
func add_table(db: TreeItem, table_name: String):
	...
	table_item.add_button(
		0,
		load("res://addons/gdsql/img/table_edit.svg"),
	...
	for col in table_columns:
		...
		for i in properties.size():
			if col.get(properties[i], false):
				col_item.add_button(
					0,
					load(
						"res://addons/gdsql/img/word_%s.svg" \
								% (properties[i] as String).to_lower(),
					),
		...
		if col.get("Index", false):
			col_item.add_button(
				0,
				load("res://addons/gdsql/img/word_in.svg"),
				ITEM_BUTTON_INDEX.COLUMN_PROPERTY,
				true,
				tr("Indexed"),
			)
...
func deal_password_before_table_cmd_3(
		item: TreeItem,
		db_name: String,
		table_name: String,
		data_path: String,
		try_password: String,
		pass_callback: Callable,
		fail_callback: Callable = Callable(),
		lock_if_already_passed: bool = false,
):
	var password_dict_obj = GDSQL.DictionaryObject.new(
		{ tr("Password"): "" },
...
func _on_button_clicked(item: TreeItem, column: int, id: int, _mouse_button_index: int) -> void:
	if column == 0:
		match id:
			...
			ITEM_BUTTON_INDEX.QUICK_SEARCH:
				var exe_select = func():
					mgr.send_to_editor_and_execute.emit(
						item.get_meta("display_name"),
						{
							"cmd": "select",
			...
			ITEM_BUTTON_INDEX.ENCRYPT:
				...
				var table_name = item.get_meta("table_name") if item_is_table else ""
				var data_path = item.get_meta("data_path")
				deal_password_before_table_cmd_3(
					item,
					item.get_meta("db_name"),
					table_name,
					data_path,
					"",
					Callable(),
				...
				var open_dialog = func():
					mgr.create_confirmation_dialog(
						tr("Are you sure to Drop table `%s`.`%s`? Config file and data file of this table will be moved to trash.") % \
								[db_name, table_name],
						drop_table_from_config.bind(db_name, table_name),
					)
				...
				var open_dialog = func():
					mgr.create_confirmation_dialog(
						tr("Are you sure to Truncate table `%s`.`%s`?") % \
								[db_name, table_name],
						truncate_table_from_config.bind(db_name, table_name),
					)
				deal_password_before_table_cmd(item, "", open_dialog)
		"Show in File Manager":
			var item := get_selected()
...
func _on_popup_menu_database_index_pressed(index: int) -> void:
	match popup_menu_database.get_item_text(index):
		...
		"Alter Schema...":
			var item := get_selected()
			if item:
				deal_password_before_table_cmd(
					item,
					"",
			...
			if item:
				...
				var open_dialog = func():
					mgr.create_confirmation_dialog(
						...
						% db_name,
						drop_db_from_config.bind(db_name),
					)
				deal_password_before_table_cmd(item, "", open_dialog)
		"Refresh All":
			refresh()
...
func _on_popup_menu_password_index_pressed(index):
	match popup_menu_password_of_table.get_item_text(index):
		"Set Password":
			...
			var password_dict_obj_1 = GDSQL.DictionaryObject.new(
				{ tr("Password"): "" },
				{ tr("Password"): { "hint": PROPERTY_HINT_PASSWORD } },
			)
			var password_dict_obj_2 = GDSQL.DictionaryObject.new(
				{ tr("Password"): "" },
