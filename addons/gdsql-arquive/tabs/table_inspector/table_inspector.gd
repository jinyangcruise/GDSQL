...
func _ready() -> void:
	...
	table.column_tips = [
		...
		"Is Auto Increment",
		"Is Indexed",
		"Default Value (Expression Supported)",
		"Comment",
	] as Array[String]
...
func _gen_row() -> GDSQL.DictionaryObject:
	...
	row.set_custom_display_control(
		"Data Type",
		label_data_type,
		update_callback.bind("Data Type", weakref(row), GDSQL.DataTypeDef.DATA_TYPE_NAMES),
		true,
	)
	row.set_custom_display_control(
		"Hint",
		label_hint,
		update_callback.bind("Hint", weakref(row), GDSQL.DataTypeDef.PROPERTY_HINT_NAMES),
		true,
	)
	return row
...
func _on_button_enter_password_pressed():
	if update_total_data_count != null:
		GDSQL.WorkbenchManager.request_user_enter_password.emit(schema, table_name, "", update_total_data_count)


func _on_button_show_in_file_manager_pressed():
	if data_file_path != "":
		var path = GDSQL.GDSQLUtils.globalize_path(data_file_path.split("(")[0])
		OS.shell_show_in_file_manager(path, true)


func _on_button_open_config_pressed():
	if schema != "" and table_name != "":
		var path = GDSQL.GDSQLUtils.globalize_path(GDSQL.RootConfig.get_table_config_path(schema, table_name))
		OS.shell_open(path)
