...
func _on_button_apply_pressed() -> void:
	...
	if _password != line_edit_password_again.text:
		return GDSQL.WorkbenchManager.create_accept_dialog(tr("Must input the same password again!"))
	...
	GDSQL.WorkbenchManager.user_confirm_add_table.emit(schema, curr_table_name, comments, _password, valid, column_infos, name)
