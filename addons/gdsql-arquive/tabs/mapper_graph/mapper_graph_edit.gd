...
func add_item(
	...
	text_enum_suggestion.ready.connect(
		func():
	...
	ob_link_type.item_selected.connect(
		func(index):
	...
	association_class_name.text_changed.connect(
...
func update_item_columns(graph_node: GraphNode, new_columns: Array):
	...
	if need_redraw:
		graph_node.redraw_slots_finished.connect(
			func():
...
func include_file(
		file_path: String,
		p_add_frame: bool = true,
		p_path = "/root",
		pos_offset = null,
		first_node_position_offset = null,
		p_name = "",
		depth = 0,
		include_connections = [],
		p_deal_include_connections = true,
		p_adjust_viewport = true,
		p_parent_frame = null,
):
	if file_path == "":
		return
	...
	var added_nodes = await _load_nodes(
		nodes,
		cons,
		pos_offset,
		false,
		...
		for i: String in connection_nodes:
			if connection_nodes[i] == null:
				EditorInterface.get_editor_toaster().push_toast(
					tr("Cannot find a GraphNode! You may have delete it!"),
...
func ensure_control_visible(p_control: GraphElement, animation_duration: float = 0) -> void:
	...
	if is_zero_approx(animation_duration):
		...
		tween.tween_property(
			self,
			"scroll_offset",
			p_control.position_offset * zoom - get_rect().get_center() + p_control.size / 2 * zoom,
			animation_duration,
		)
...
func _load_nodes(
		...
		p_path: String = "/root",
		p_include_file_index: int = -1,
		include_connections: Array = [],
		p_parent_frame = null,
):
	var ret_nodes = []
