...
func alter_database(old_name: String, new_name: String) -> Error:
	_begin_time = Time.get_unix_time_from_system()
	return await _exec_with_password_guard(
		"ALTER DATABASE",
		old_name,
...
func drop_table(db_name: String, table_name: String) -> Error:
	...
	return await _exec_with_password_guard(
		"DROP TABLE",
		db_name,
		func(): return _drop_table_impl(db_name, table_name),
		table_name,
	)
...
func truncate_table(db_name: String, table_name: String) -> Error:
	...
	return await _exec_with_password_guard(
		"TRUNCATE TABLE",
		db_name,
		func(): return _truncate_table_impl(db_name, table_name),
		table_name,
	)
...
## 处理密码逻辑，参考 base_dao.gd 的 _handle_default_password。
...
func _alter_database_impl(old_db_name: String, new_db_name: String) -> Error:
	...
	if dir == null:
		msgs.push_back(
			tr("Failed! Cannot open config root %s dir! Err: %s.") %
...
func _drop_database_impl(db_name: String) -> Error:
	...
	if _is_editor():
		...
		if dek64:
			...
			var tmp_file_path = "user://%s.%s.%s.dek" % [
				db_name,
...
func _create_table_impl(
	...
	if conf_dir == null:
		msgs.push_back(
			tr("Failed! Cannot open database config dir %s! Err: %s.") \
...
func _alter_table_impl(
		...
		for i in column_infos:
			...
			if old_columns_map.has(col_name):
				...
				if old_columns_map[col_name]["Data Type"] != i["Data Type"]:
					warnings.push_back(
						tr("Column [%s] data type changed from [%s] to [%s], datas will be converted!") % \
	...
	var apply = func() -> Error:
		...
		for i: Dictionary in old_values:
			...
			for c in column_infos:
				...
				if default_value == null:
					default_value = GDSQL.DataTypeDef.DEFAULT_VALUES[c["Data Type"]]
		...
		var return_column_name = func(v):
			return v["Column Name"]

		# 设置缓存
		new_table_data_file.set_indexed_props(
			column_infos.filter(
				func(v): return v.Index
			).map(
				return_column_name,
			),
		)
