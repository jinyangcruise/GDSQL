...
func limit(a_offset: int, a_limit: int) -> BaseDao:
	...
	assert(_assert("limit", a_offset >= 0 and a_limit > 0, "offset must not less than 0 and limit must larger than 0"))
	__offset = a_offset
	__limit = a_limit
	return self
...
func left_join(db: String, table: String, alias: String, cond: String, password: String) -> BaseDao:
	...
	assert(_assert("left_join", alias != __table_alias and (__left_join == null or not __left_join.chain_has_alias(alias)), 
...
func ___select(path: String, fill_primary_key: String = ""):
	...
	for s in __select:
		if s == "*":
			...
			if alias == __table_alias:
				real_select.append_array(
					__get_table_columns(__database, __table, __table_alias, all_datas).map(fill_select_name.bind(alias)))
			else:
			...
			if m != null and m.get_string() == s:
				assert(_assert("___select", __left_join == null, "must specify table alias name in select fields if using left join"))
				...
				if column != null and !column.is_empty():
					...
					assert(_assert("___select", not all_datas[__table_alias].is_empty() and all_datas[__table_alias][0].has(s),
				...
				if m:
					...
					if column != null and !column.is_empty():
						...
						if s == __table_alias + "." + field:
							assert(_assert("___select", not all_datas[__table_alias].is_empty() and all_datas[__table_alias][0].has(field),
					...
					if s.contains(alias + "."):
						...
						if m:
							...
							if column != null and !column.is_empty():
								...
								if s == alias + "." + field:
									assert(_assert("___select", not all_datas[alias].is_empty() and all_datas[alias][0].has(field),
...
func __get_table_columns(db_path, table_name, table_alias, all_datas: Dictionary = {}):
	...
	if columns == null or columns.is_empty():
		...
		assert(_assert("__get_table_columns", not all_datas.get(table_alias, []).is_empty(), 
			"db: [%s] table [%s] cannot get head because there is no defination of this table or any data of this table" \
			% [db_path, table_name]))
