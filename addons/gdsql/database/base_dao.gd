...
var __simplify_exp_cache: Dictionary ## 【外部请勿使用】简化表达式的cache
...
## 将复杂表达式转为简单表达式 
func _simplify_expression(expression: String, sql_input_names: Dictionary = {}, sql_inputs: Array = []):
	...
	var contain_lacking = _collect_lack_table(possible_sql)
	...
	var ret = [expression, nested_sql_queries]
	# 只有不缺表的才进行缓存，否则缺表的dao不能query
	if not contain_lacking:
		__simplify_exp_cache[bak] = ret
	return ret
...
loop_index: int, curr_row: Dictionary, head: Array, table_definations: Dictionary) -> bool:
	if loop_index == loop_tables.size():
		...
		if all_datas[table].size() > 0:
			...
			#     }
			# }
			var input_names = {} # input names based on curr_row
			for t in curr_row:
				if not input_names.has(t):
					input_names[t] = {}
				input_names[t][true] = 0
				for k in curr_row[t]:
			...
			if not input_names.has(table):
				input_names[table] = {}
			input_names[table][true] = 0
			for f in table_definations[table]:
				if not input_names.has(f["Column Name"]):
					input_names[f["Column Name"]] = {}
				input_names[f["Column Name"]][table] = 0
			...
			for row in all_datas[table]:
				...
				var check_result = condition_wrapper.cond(simple_expression[0], 
					input_names, simple_expression[1]).check([acc_row, __inputs])
...
cond: String, all_table_defination: Dictionary, all_datas: Dictionary, curr_dependency: Dictionary = {}):
	...
	if cond != '':
		...
		expression.parse(simple_expression[0])
		# 主键或索引有关的操作， 例如一下cond：
		# "id == 1"
		# "1 == id"
		# "id in [1, 2, 3]"
...
func ___select(path: String, fill_primary_key: String = ""):
	...
	for t in all_table_defination:
		...
		__final_input_names[t][true] = 0 # true: 0
		for f in all_table_defination[t]:
			if not __final_input_names.has(f["Column Name"]):
				__final_input_names[f["Column Name"]] = {}
			__final_input_names[f["Column Name"]][t] = 0 # 'y': 0
	...
	if cond == "" or ret.is_empty():
		...
		for data in ret:
			...
			var check_result = condition_wrapper.check([data, __inputs])
			if not condition_wrapper.get_lacking_tables().is_empty():
				if __collect_lack_table_enabled:
					__lack_table.append_array(condition_wrapper.get_lacking_tables())
					return null
			...
			for j in group_key:
				if j is Array:
					...
					var value = GDSQLUtils.evaluate_command_with_sql_expression(
						null, j, [], [], __final_input_names, [m_data, __inputs],
			...
			for field in real_select:
				...
				if not dealed:
					...
					var value = GDSQLUtils.evalute_command_with_agg(agg_func_obj, 
						simple_expression[0], [], [], __final_input_names, [data, __inputs], simple_expression[1])
