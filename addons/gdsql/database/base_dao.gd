...
var __CONF_MANAGER: ConfManagerClass = Engine.get_singleton("ConfManager")
...
## 手动提交（保存到文件）
func commit() -> void:
	assert(__database != "", "database is empty")
	assert(__table != "", "table name is empty")
...
func select(someting: String, need_head: bool) -> BaseDao:
	...
	if matches:
		...
		var start = 0
		for i in matches:
			var field_str = someting.substr(start, i.get_start() - start).strip_edges() # 知道逗号的起始位置，就可以截取逗号前的位置到上一个逗号的结束位置
			start = i.get_end()
			
			# 有可能取了别名，例如t_user.icon(1, 2, \"a, b\") as iii
			var m = regex_2.search(field_str)
			if m:
				field_str = field_str.substr(0, m.get_start(0)) # 实际要求的式子，例子中的t_user.icon(1, 2, \"a, b\")
	...
	# 没有逗号分割，*或者某个单独的字段
		...
		if m:
			someting = someting.substr(0, m.get_start(0)) # 实际要求的式子，例子中的t_user.icon(1, 2, \"a, b\")
...
func select_same() -> BaseDao:
	assert(__parent_union != null, "must have parent union!")
	assert(__cmd == "", "already set command %s" % __cmd)
	__cmd = "select"
...
func values(data: Dictionary) -> BaseDao:
	assert(__cmd.begins_with("insert") or __cmd.begins_with("replace"), "'values' can only be used after 'insert' or 'replace'")
	__data = data
	return self
	
func sets(data: Dictionary) -> BaseDao:
	assert(__cmd == "update", "'sets' can only be used after 'update'")
	__data = data
	return self
...
func where(cond: String) -> BaseDao:
	assert(__cmd == "select" or __cmd == "update" or __cmd == "delete_from", "'where' can only be used after 'select' or 'update' or 'delete_from'")
...
func limit(a_offset: int, a_limit: int) -> BaseDao:
	assert(__cmd == "select", "'limit' can only be used after 'select'")
	assert(a_offset >= 0 and a_limit > 0, "offset must not less than 0 and limit must larger than 0")
	__offset = a_offset
	__limit = a_limit
	return self
	
func on_duplicate_update(fields: Array[String]) -> BaseDao:
	assert(__cmd == "update" or __cmd == "insert_or_update", "'on_duplicate_update' can only be used after 'update'")
...
func ___loop_table_row(result: Array, all_datas: Dictionary, loop_tables: Array, loop_index: int, curr_row: Dictionary):
	if loop_index == loop_tables.size():
		var ret = curr_row.duplicate()
		# 循环到头了，依次检查每个表是否满足on条件
		for table_alias in __left_join_tables:
			var cond = __left_join_tables[table_alias][2]
		...
		for i in 10000:
			...
			for table_alias in __left_join_tables:
				var dependencies: Array = __left_join_tables[table_alias][3]
...
func ___select(path: String, fill_primary_key: String = ""):
	...
	if !__left_join_tables.is_empty():
		for table_alias in __left_join_tables:
			var t = __left_join_tables[table_alias][0]
			var p = __left_join_tables[table_alias][1]
			var conf1: ImprovedConfigFile = __CONF_MANAGER.get_conf(t, p)
...
#		__union_all.__need_head = false
	...
	if not __order_by.is_empty():
		var compare := func(a, b):
			for a_order_by in __order_by:
				...
				if t1 == "" and a is Dictionary and (a.size() == 1 or (a.size() == 2 and a.has(__ROW_POST_PROCESS__))):
					var ts: Array = a.keys()
					ts.erase(__ROW_POST_PROCESS__)
					t1 = ts[0]
				if t2 == "" and b is Dictionary and (b.size() == 1 or (b.size() == 2 and b.has(__ROW_POST_PROCESS__))):
					var ts: Array = b.keys()
					ts.erase(__ROW_POST_PROCESS__)
					t2 = ts[0]
	...
	if __left_join_tables.is_empty() and __select.size() == 1 and (__select[0] == "*" or __select[0] == __table_alias + ".*"):
		...
		for d in ret_filter:
			...
			var row = []
			for f in head:
				row.push_back(d[__table_alias][f])
			if __parent_union:
		...
		for i in fields.size():
			fields[i] = ConditionWrapper.modify_dot_to_get(fields[i]) # t.name.substr(10) 被替换为：t["name"].substr(10)
		...
		for data in ret_filter:
			...
			for field in fields:
				...
				var value = Evaluate.evaluate_command(null, field, variable_names, variable_values)
				row.push_back(value)
...
func query():
	assert(!__database.is_empty(), "database is empty")
	assert(!__table.is_empty(), "table is empty")
	assert(!__cmd.is_empty(), "command is empty")
	...
	match __cmd:
		...
		"insert_into", "insert_ignore", "insert_or_update", "replace_into":
			assert(!__data.is_empty(), "data is empty")
			assert(__primary_key != null and __primary_key != "", "primary key is empty")
		...
		"update":
			assert(!__data.is_empty(), "data is empty")
			assert(!__where.is_empty(), "where is empty")
			var ret = { "affected_rows": 0 }
			# 筛选出要更新的数据
