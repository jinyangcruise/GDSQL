		...
		if s == "*":
			...
			if m != null and m.get_string() == s:
				...
				if column != null and !column.is_empty():
					...
					assert(not all_datas[__table_alias].is_empty() and all_datas[__table_alias][0].has(s),
	...
	if __left_join == null and __select.size() == 1 and (__select[0] == "*" or __select[0] == __table_alias + ".*"):
		...
		for data in ret_filter:
			...
			for field in real_select:
				...
				var value = Evaluate.evaluate_command(null, field["name_4_computing"], variable_names, variable_values)
				row.push_back(value)
