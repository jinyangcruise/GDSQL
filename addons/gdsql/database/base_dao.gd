...
sql_static_inputs: Array = [], sql_varying_inputs: Dictionary = {}):
	if __simplify_exp_cache.has([expression, sql_input_names, sql_static_inputs]):
		return __simplify_exp_cache[[expression, sql_input_names, sql_static_inputs]]
	var bak = expression
	...
	if not contain_lacking:
		__simplify_exp_cache[[bak, sql_input_names, sql_static_inputs]] = ret
	return ret
