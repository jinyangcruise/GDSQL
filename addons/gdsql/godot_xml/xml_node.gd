...
## Determine how to organize children.
enum CHILDREN_MODE {
	## [code]{'children': {child_name: child}}[/code] will be used. Children with same name will override the previous one.
	DICT_OVERRIDE,
	## [code]{'children': {child_name: [child]}}[/code] will be used. Children with same name will be grouped in an array.
	DICT_GROUPED_BY_NAME,
	## [code]{'children': [child]}[/code] will be used. All children will be put in an array.
	ARRAY,
}
...
func to_dict(children_mode: CHILDREN_MODE) -> Dictionary:
	var output = {}
	...
	var _children = [] if children_mode == CHILDREN_MODE.ARRAY else {}
	for child in children:
		match children_mode:
			CHILDREN_MODE.DICT_OVERRIDE:
				_children[child.name] = child.to_dict(children_mode)
			CHILDREN_MODE.DICT_GROUPED_BY_NAME:
				if not _children.has(child.name):
					_children[child.name] = []
				_children[child.name].push_back(child.to_dict(children_mode))
			CHILDREN_MODE.ARRAY:
				_children.push_back(child.to_dict(children_mode))

	output["children"] = _children
