...
## Standalone UPDATE operation presentation with explicit all-row safety.
...
func build_graph_node() -> GDSQLOperationResult:
	...
	if predicate == null and not _all_rows_confirmation.button_pressed:
		return _error(
			&"GDSQL_QUERY_GRAPH_UPDATE_ALL_ROWS_CONFIRMATION_REQUIRED",
			"Enable WHERE or explicitly confirm updating every row.",
		)
	var node := GDSQLQueryGraphUpdateNode.new(
...
func _configure_fields() -> void:
	...
	_values.configure(columns, excluded)
