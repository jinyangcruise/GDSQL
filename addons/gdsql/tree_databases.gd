...
func add_db_to_config(db_name: String, path: String, save: bool, id: String):
	if _config_file.has_section(db_name) or _tmp_config.has_section(db_name):
		var dialog := AcceptDialog.new()
		dialog.dialog_text = "failed! database %s already exist!" % db_name
		add_child(dialog)
...
func refresh() -> void:
	...
	for data in databases:
		...
		for t in data["table_items"]:
			popup_menu_create_table_like_tables.add_item((t as TreeItem).get_text(0))
			popup_menu_create_table_like_table_item.add_item((t as TreeItem).get_text(0))
...
func _on_popup_menu_database_index_pressed(index: int) -> void:
	printt(popup_menu_database.get_item_text(index))
