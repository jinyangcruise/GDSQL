...
func _draw():
	...
	draw_dashed_line(Vector2(size.x, 0) + Vector2(margin_right, margin_top) - Vector2(width/2, 0), 
		Vector2(size.x, size.y) + Vector2(margin_right, margin_bottom) - Vector2(width/2, 0), color, a_width, a_width, false)
