...
func _ready() -> void:
	...
	# TODO DictionaryObject增加一种自定义装饰器（用于显示该属性），可以让用户自定义显示效果。set_custom_xxx(property, control)。get_custom_xxx(property) -> Control
	var row := DictionaryObject.new([table.columns, ["idnew_table", TYPE_INT, PROPERTY_HINT_NONE , "", true, true, false, true, "", ""]], _hint_string)
	# 为Data Type和Hint设置自定义显示控件和数据绑定逻辑，否则会默认显示为一个整数，难以让用户分辨
	var update_callback := func(new_value, property, dict_obj_ref: WeakRef, readable_map: Array):
		var dict_obj = dict_obj_ref.get_ref() as DictionaryObject
		if dict_obj:
			dict_obj.get_custom_display_control(property) # TODO 使用原始还是复制品？避免queuefree、重复add_child
			#label.text = (readable_map[new_value] as String).split(":")[0]
			# TODO 
			
	var label_data_type := Label.new()
	#label_data_type.text = types[TYPE_INT]
	row.set_custom_display_control("Data Type", label_data_type, update_callback.bind("Data Type", weakref(row), types))
	
	var label_hint := Label.new()
	row.set_custom_display_control("Hint", label_hint, update_callback.bind("Hint", weakref(row), property_hints))
...
func _on_button_new_column_pressed() -> void:
	var row := DictionaryObject.new([table.columns, ["new_tablecol", TYPE_INT, PROPERTY_HINT_NONE, "", false, false, false, false, "", ""]], _hint_string)
	datas.push_back(row)
	table.datas = datas
