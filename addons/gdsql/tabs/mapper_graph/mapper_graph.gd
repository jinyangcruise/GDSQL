...
func get_helpers_left_join_cmds(
		node: GraphNode,
		to_from_map: Dictionary,
			...
			if ffnode_is_helper_node and graph_edit.is_helper_node(fnode):
				...
				for ffnode in to_from_map[fnode]:
					for i in to_from_map[fnode][ffnode]:
						...
						# 因为不能连接多个实体表。
						ret.where.push_front(
							"%s.%s == #{%s}" % [
								get_alias_func.call(fnode),
								i[1],
								i[0],
							],
						)
...
func popup_generate_dialog(xml_map, mapper_map, entity_map):
	...
	line_edit_path.text_changed.connect(
		func(t):
	...
	_generate_dialog.custom_action.connect(
		func(action):
			match action:
				"Inspect Selected":
					var arr_content = []
...
func popup_saveas_dialog(access: int, item: TreeItem, dir: String):
	...
	editor_file_dialog.ready.connect(
		func():
			if dir != "":
	...
	editor_file_dialog.file_selected.connect(
		self.comfirm_save.bind(item, editor_file_dialog),
		CONNECT_DEFERRED,
	)
	...
	for i in arr_content:
		...
		editor.ready.connect(
			func():
				...
				code_edit.get_v_scroll_bar().value_changed.connect(
					func(v):
						for a_bar: VScrollBar in arr_v_scroll_bar:
							if a_bar != code_edit.get_v_scroll_bar():
								a_bar.value = v
				)
				editor.text_editor.caret_changed.connect(
					func():
				...
				if show_diff:
					...
					code_edit.gutter_clicked.connect(
						_on_gutter_clicked.bind(
							index == 0,
							arr_editor,
							arr_content[index],
						),
...
func default_val(type: int) -> String:
	return var_to_str(GDSQL.DataTypeDef.DEFAULT_VALUES[type]).replace('""', "''")
...
func update_popup_panel_node_list_size():
	...
	var max_height = min(
		last_item_rect.end.y + line_edit_filter_node_list.size.y +
...
func _on_option_button_choose_path_item_selected(access: int, extra_line_edit = null, parent_dialog = null) -> void:
	...
	editor_file_dialog.dir_selected.connect(
		func(path: String):
	...
	editor_file_dialog.canceled.connect(
		func():
			editor_file_dialog.queue_free(),
		CONNECT_DEFERRED,
	)
...
func _generate(nodes: Array):
	...
	for i in graph_edit.get_connection_list():
		...
		assert(
			to_node.get_meta("extra_enabled", false),
						...
						if option_button_link.selected == LINK_WAY.NESTING_SELECT:
							...
							if ainfo.is_helper:
								...
								method = "select_%s_by_%s" % [
									info.link_prop_type.to_snake_case(),
									"_".join(by),
								]
			...
			for i in arr_prop_type:
				...
				if e_map.has(i[0]):
					...
					if old_p_map.comment != p_map.comment:
						push_error(
							"Detect inconsistent comment of property: `%s.%s`, 1: `%s`, 2: `%s`." % [
								en_ns,
								i[0],
								old_p_map.comment,
								p_map.comment,
					...
					if old_p_map.define != p_map.define:
						push_error(
							"Detect inconsistent defination of property: `%s.%s`, 1: `%s`, 2: `%s`." % [
								en_ns,
								i[0],
								old_p_map.define,
								p_map.define,
					...
					if old_p_map.getset != p_map.getset:
						push_error(
							"Detect inconsistent getset of property: `%s.%s`, 1: `%s`, 2: `%s`." % [
								en_ns,
								i[0],
								"".join(old_p_map.define),
								"".join(p_map.define),
							],
						)
				else:
		...
		# nesting resultMap 需要使用left join，把所有表形成一条sql
			...
			for node_name in linked_nodes:
				...
				if leading:
					leading = false
					if option_button_result_map_mode.selected == RESULT_MAP_MODE.SIMPLEST:
						vo = split_for_long_content(
							"select %s from %s.%s %s" % [
								", ".join(all_table_alias_simplest_fields),
								leading_db_name,
								leading_table_name,
								t_alias,
							],
	...
	if is_left:
		if gutter == _get_commit_line_gutter(left_edit):
			var insert_line = GDSQL.DiffHelper.merge_delete_line_by_mapping(
				line,
				right_edit.get_line_count(),
				right_edit.get_meta("gutter_mapping"),
			)
		...
		if gutter == _get_commit_line_gutter(right_edit):
			var insert_line = GDSQL.DiffHelper.merge_insert_line_by_mapping(
				line,
				left_edit.get_line_count(),
				left_edit.get_meta("gutter_mapping"),
			)
...
func _check_table_change(item: TreeItem) -> bool:
	...
	if node and node is GraphNode and node.has_meta("data") and node.get_meta("data").get("__table_item", false):
		...
		if columns_define.is_empty():
			item.add_button(
				0,
				get_theme_icon(&"StatusWarning", &"EditorIcons"),
				NODE_LIST_TREE_ITEM_BUTTON.TABLE_NOT_EXIST,
				false,
				tr("This table is not exist!"),
			)
			...
			if old_comment != new_comment:
				item.add_button(
					0,
					get_theme_icon(&"VisualShaderNodeComment", &"EditorIcons"),
			...
			if not column_diffs.is_empty():
				...
				item.set_meta(
					"_meta_change_%d" % TABLE_CHANGE_TYPE.META_COLUMN_NAME_CHANGE,
			...
			if not change_message.is_empty():
				item.add_button(
					0,
					load("res://addons/gdsql/img/file_type_diff_icon.svg"),
